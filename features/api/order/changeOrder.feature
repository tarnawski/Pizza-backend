Feature: Change order status
  In order to have possibility mark order as realized
  As a login user
  I need to be able to change order status
  ...
  @cleanDB
  Scenario: Change status of order
    ...
    When I send a PUT request to "/api/orders/1" with body:
    """
    {
      "realized": true
    }
